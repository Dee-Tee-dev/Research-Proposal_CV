...
def _font(size: int, bold: bool = False):
    """Load a suitable font on Windows, macOS, or Linux."""
...
def validate_queue(
    ...
    if expected_rows is not None and len(frame) != expected_rows:
        raise ValueError(
            f"Review queue has {len(frame)} rows, "
...
def create_review_sheets(
    ...
    for page_index in range(pages):
        ...
        page_rows = frame.iloc[
            start : start + columns * rows
        ]

        for local_index, (_, record) in enumerate(
            page_rows.iterrows()
        ):
            ...
            title = (
                ...
                f"{record['income_quartile']} | "
                f"auto={record['metric_value']}"
            )
            ...
            if not image_path.exists():
                raise FileNotFoundError(
                    f"Missing review image: {image_path}"
                )
            ...
            text_width_chars = max(
                25,
                (tile_width - image_size[0] - 55) // 9,
            )
        ...
        output = output_dir / (
            f"qwen_review_{page_index + 1:02d}"
            f"_of_{pages:02d}.png"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Create contact sheets for Divya's "
            "84-image Qwen caption review."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "paper/review/divya_qwen_caption_review.csv"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "results/divya/qwen_review_sheets"
        ),
    )
    ...
    print(
        f"Created {len(outputs)} Qwen review sheets "
