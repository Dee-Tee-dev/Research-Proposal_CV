...
CLIP used its checkpoint processor (short-edge resize to 224 pixels and 224 x 224 centre crop); BLIP used its 384 x 384 processor. Qwen used a fixed 256-visual-token budget (200,704 pixels). InternVL used its checkpoint dynamic image preprocessing. YOLO-World retained aspect ratio through its model preprocessing. The fixed manifest, image IDs, prompts, outputs, and run metadata are saved in the repository.

### 2.2 Models and tasks

CLIP ViT-B/32 (Radford et al., 2021) performed six-way zero-shot classification with the proposal prompt `a photo of a household {label}`. The exact prompt was rerun after integration; this corrected run supersedes the earlier pilot wording. BLIP-base (Li et al., 2022) generated an unprompted caption and a caption starting with the label-free prefix `the main household object in this image is`.
